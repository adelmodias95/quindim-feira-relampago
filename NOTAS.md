...
- O frontend em Nuxt 4 foi escrito pelo agente de IA e revisado por mim. É uma tela só, atendendo a seção 7.1: mostra os livros com preço e disponibilidade, permite escolher a quantidade respeitando o limite de 3 por SKU e o que está disponível, reserva, acompanha o tempo restante em contagem regressiva e confirma, exibindo o pedido com o desconto por linha. O navegador nunca fala direto com a API. As chamadas passam por rotas de servidor do Nuxt, e a URL da API fica só no servidor. Os ids da reserva e do pedido ficam na URL, então atualizar a página não perde o estado. A tela termina em `aguardando_pagamento` porque, pela RN-09, só o webhook do provedor muda o status do pedido.
- Os testes foram escritos pelo agente de IA e revisados por mim. São dois grupos, como a seção 7.3 pede. Os de unidade cobrem o desconto e o rateio sem banco: o exemplo da spec, o caso em que o meio centavo sobe, a faixa contando unidades e não linhas, o empate no resto indo para a primeira linha, a entrada não ser alterada e nenhum valor ser float. O último deles percorre as 1023 combinações possíveis do catálogo conferindo que a soma dos descontos das linhas é exatamente o desconto total — foi assim que eu validei o rateio à mão na sexta, e virou teste. O de concorrência sobe 20 requisições simultâneas com `ThreadPoolExecutor` contra o MongoDB de verdade: no `QND-005`, de estoque 1, exatamente uma reserva é criada e dezenove levam 409; no `QND-004`, de estoque 3, exatamente três passam. Só `urllib` e `json` da biblioteca padrão, sem acrescentar dependência.
